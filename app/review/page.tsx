...
function stripTypes(src: string): string {
  return src
    ...
    .replace(/:\s*[\w<>\[\]|&, .?]+(?=\s*=)/g, '')
    // Remove generic type params on functions/calls  <T>  <T, U>
...
function ConsolePanel({ code, language, isOpen, onToggle }: {
  ...
  const run = useCallback(() => {
    ...
    // Detect JSX — browser can't eval it natively
    const hasJsx = /<\s*[A-Za-z][A-Za-z0-9.]*[\s/>]/.test(code) || /return\s*\([\s\S]*</.test(code);
    if (hasJsx) {
      setLines([{
        id: lineId++,
        type: 'warn',
        args: ['JSX detected — the browser sandbox can\'t run JSX directly.'],
      }, {
        id: lineId++,
        type: 'system',
        args: ['Tip: extract pure logic functions (no JSX) and run those instead.'],
      }]);
      setRunning(false);
      return;
    }
  ...
  return (
    <div style={{ flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.07)', background: '#080808', display: 'flex', flexDirection: 'column' }}>
      ...
      <div
        ...
        style={{ height: 40, display: 'flex', alignItems: 'center', padding: '0 12px', gap: 10, cursor: 'ns-resize', position: 'relative', flexShrink: 0 }}
      >
        {/* Grip */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'default' }}>
          <div style={{
            width: 7, height: 7, borderRadius: '50%', flexShrink: 0, transition: 'all 0.2s',
            background: running ? '#00FF85' : lines.some(l => l.type === 'error') ? '#FF0099' : 'rgba(255,255,255,0.15)',
            boxShadow: running ? '0 0 8px rgba(0,255,133,0.7)' : 'none',
            animation: running ? 'consolePulse 1s ease-in-out infinite' : 'none',
          }} />
          <span style={{ fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            console
          </span>
          {lines.length > 0 && (
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.06)', padding: '1px 6px', borderRadius: 100 }}>
              {lines.length}
            </span>
          )}
        </div>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, cursor: 'default' }}>
          {lines.length > 0 && (
            <button onClick={() => setLines([])} style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.25)', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '3px 9px', cursor: 'pointer' }}>
              ✕ clear
            </button>
          )}
          <button
            ...
            style={{
              ...
              color: running ? 'rgba(0,255,133,0.4)' : '#0D0D0D',
              background: running ? 'rgba(0,255,133,0.1)' : '#00FF85',
              border: 'none', borderRadius: 6, padding: '4px 12px', cursor: running ? 'not-allowed' : 'pointer',
              display: 'flex', alignItems: 'center', gap: 5,
              boxShadow: running ? 'none' : '0 0 14px rgba(0,255,133,0.35)',
              letterSpacing: '0.05em', transition: 'all 0.15s',
            }}
          >
            {running
              ? <><span style={{ width: 10, height: 10, border: '1.5px solid rgba(0,255,133,0.2)', borderTopColor: '#00FF85', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} /> running…</>
              : <>▶ run</>
            }
          </button>
          <button
            onClick={onToggle}
            style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.2)', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '3px 9px', cursor: 'pointer' }}
          >
            {isOpen ? '↓ hide' : '↑ show'}
          </button>
        </div>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div
            style={{ height: panelH, overflow: 'hidden', display: 'flex', flexDirection: 'column', borderTop: '1px solid rgba(255,255,255,0.05)' }}
          ...
          >
            {lines.length === 0 && (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, color: 'rgba(255,255,255,0.15)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                {running
                  ? <><span style={{ width: 14, height: 14, border: '2px solid rgba(0,255,133,0.2)', borderTopColor: '#00FF85', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} /> executing…</>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
              {lines.map((line, i) => (
                <div
                  key={line.id}
                  style={{
                    display: 'flex', alignItems: 'baseline', padding: '2px 0',
                    background: line.type === 'error' ? 'rgba(255,0,153,0.04)' : line.type === 'warn' ? 'rgba(245,158,11,0.03)' : 'transparent',
                  }}
                >
...
function ReviewPageInner() {
  ...
  const fixIssue = (id: string) => { setIssues(prev => prev.map(i => i.id === id ? { ...i, fixed: true } : i)); setActiveId(null); };
  const applyFix = (issue: Issue, fixedCode: string) => { editorApiRef.current?.applyFix(issue, fixedCode); };
  ...
  const liveScore = (() => {
    ...
    // Score scales from result.score up to 100 as issues get fixed
    // Only hits 100 when every issue is resolved
    const base = result.score;
    const ceiling = 100;
    return Math.round(base + (ceiling - base) * (fixed / total));
  })();
  const grouped      = { error: activeIssues.filter(i => i.severity === 'error'), warning: activeIssues.filter(i => i.severity === 'warning'), suggestion: activeIssues.filter(i => i.severity === 'suggestion') };
  const severityLabel = { error: 'Errors', warning: 'Warnings', suggestion: 'Suggestions' };
  const severityColor = { error: '#FF0099', warning: '#F59E0B', suggestion: '#1E90FF' };
    ...
    <main suppressHydrationWarning style={{ height: '100vh', overflow: 'hidden', background: '#0D0D0D', color: '#FFFFFF', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        {!loading && result && (
          <div className="topbar-badges">
            {(['error', 'warning', 'suggestion'] as const).map(sev => {
              ...
              return <span key={sev} className="sev-badge" style={{ color: severityColor[sev], background: `${severityColor[sev]}18`, border: `1px solid ${severityColor[sev]}35` }}>{count} {sev}{count !== 1 ? 's' : ''}</span>;
            })}
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="loading-overlay">
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, textAlign: 'center' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', margin: 0, fontFamily: 'var(--font-mono)' }}>running AI analysis</p>
                </div>
              </div>
            </div>
          )}
          ...
          {error && (
            <div style={{ position: 'absolute', inset: 0, zIndex: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
              <p style={{ color: '#FF0099', fontSize: 14, margin: 0, fontFamily: 'var(--font-mono)' }}>{error}</p>
              <button onClick={() => router.push('/')} style={{ fontSize: 12, color: '#1E90FF', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', textDecoration: 'underline' }}>← try again</button>
            </div>
          )}

          {/* Monaco editor — takes all remaining vertical space */}
        ...
        <div className="sidebar">
          {result && <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}><HealthScore score={liveScore} issueCount={issues.length} fixedCount={fixedCount} /></div>}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, minHeight: 0 }}>
            ...
            {!loading && activeIssues.length === 0 && fixedCount === 0 && !error && (
            ...
            <AnimatePresence>
              {!loading && (['error', 'warning', 'suggestion'] as const).map(sev => {
                ...
                return (
                  <motion.div key={sev} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: severityColor[sev] }}>{severityLabel[sev]}</span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {sevIssues.map((issue, i) => (
                        <IssueCard key={issue.id} issue={issue} index={i} isActive={activeId === issue.id} onClick={() => setActiveId(prev => prev === issue.id ? null : issue.id)} onFix={() => fixIssue(issue.id)} onApplyFix={issue.fixedCode ? applyFix : undefined} />
                      ))}
                    </div>
                  </motion.div>
                );
            ...
            {fixedCount > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {issues.filter(i => i.fixed).map((issue, i) => <IssueCard key={issue.id} issue={issue} index={i} isActive={false} onClick={() => {}} onFix={() => {}} />)}
                </div>
              </div>
            )}
          </div>
      ...
      <style>{`
        ...
        .topbar { height: 50px; flex-shrink: 0; border-bottom: 1px solid rgba(255,255,255,0.06); background: rgba(13,13,13,0.98); display: flex; align-items: center; padding: 0 16px; gap: 10px; z-index: 10; position: relative; }
        .back-btn { font-size: 12px; font-weight: 600; color: rgba(255,255,255,0.3); background: none; border: none; cursor: pointer; font-family: var(--font-display); padding: 4px 8px; border-radius: 6px; transition: color 0.15s; letter-spacing: 0.06em; }
        ...
        .file-dot { width: 6px; height: 6px; border-radius: 50%; background: #00FF85; box-shadow: 0 0 8px rgba(0,255,133,0.6); flex-shrink: 0; }
        .file-name { font-size: 12px; color: rgba(255,255,255,0.45); font-family: var(--font-mono); }
        .topbar-actions { display: flex; gap: 4px; margin-left: 8px; }
        .action-btn { font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.3); background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07); border-radius: 6px; padding: 4px 10px; cursor: pointer; transition: all 0.15s; font-family: var(--font-mono); }
        .action-btn:hover { color: #FFFFFF; background: rgba(255,255,255,0.08); }
        .topbar-badges { margin-left: auto; display: flex; gap: 6px; align-items: center; }
        .sev-badge { font-size: 10px; font-weight: 700; padding: 3px 10px; border-radius: 100px; font-family: var(--font-mono); letter-spacing: 0.03em; }
